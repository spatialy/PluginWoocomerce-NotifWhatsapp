...
// catatan programmer
// ditambahakan kata woowa_ agar menghindari penggunaan librari yg sama di plugin publik orang lain
// helper functions
// -----------------------------------------------------------------------------
// get html dom from file
// $maxlen is defined in the code as PHP_STREAM_COPY_ALL which is defined as -1.
function woowa_file_get_html($url, $use_include_path = false, $context=null, $offset = 0, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	// Ensure maximum length is greater than zero
	if($maxLen <= 0) { $maxLen = MAX_FILE_SIZE; }

	// We DO force the tags to be terminated.
	$dom = new woowa_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
	...
	$contents = woowa_file_get_contents2($url, $use_include_path, $context, $offset, $maxLen);
...
function woowa_file_get_contents2($url,$use_include_path='', $context='', $offset='', $maxLen=''){
	$curl = curl_init();
...
function woowa_str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	$dom = new woowa_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
// dump html dom tree
function woowa_dump_html_tree($node, $show_attr=true, $deep=0)
{
	$node->dump($node);
}
 ...
 * @package PlaceLocalInclude
 */
class woowa_simple_html_dom_node
{
	/**
	 * Node type
	 *
...
class woowa_simple_html_dom
{
	...
	protected function prepare($str, $lowercase=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new woowa_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new woowa_simple_html_dom_node($this);
			++$this->cursor;
	...
	function createElement($name, $value=null) {return @woowa_str_get_html("<$name>$value</$name>")->first_child();}
	function createTextNode($value) {return @end(woowa_str_get_html($value)->nodes);}
	function getElementById($id) {return $this->find("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
	function getElementByTagName($name) {return $this->find($name, 0);}
